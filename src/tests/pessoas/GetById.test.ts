...
describe('Pessoas GetById', () => {
    ...
    beforeAll(async () => {
        ...
        const result = await testServer.post('/pessoas').send({ nomeCompleto: "Nome completo", email: "getbyid@jet.com", cidadeId: cidadeId });
    ...
    it('Deletar registro inexistente', async () => {
        const res = await testServer.get('/pessoas/99999999');
    ...
    it('Id inválido (float)', async () => {
        const res = await testServer.get('/pessoas/1.5');
    ...
    it('Id inválido (string)', async () => {
        const res = await testServer.get('/pessoas/test');
    ...
    it('Id inválido (boolean)', async () => {
        const res = await testServer.get('/pessoas/true');
    ...
    it('Id inválido (igual a zero)', async () => {
        const res = await testServer.get('/pessoas/0');
